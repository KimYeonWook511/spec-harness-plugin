...
4. **필수 컨벤션·ADR 위반** — 주입된 필수 코딩 컨벤션이나 ADR을 명백히 어겼는가. (필요하면 `Read`로 전문 확인)
5. **코드에 spec ID 잔존** — 이 step이 추가·수정한 주석·Javadoc·`@DisplayName`·테스트 메서드명에
   `FR-###`·`SC-###`·`ADR-L#` 같은 spec·ADR 내부 식별자가 남아 있는가. 남아 있으면 `retryable_error`로
   지적하고, ID를 빼되 의미는 문장으로 살리라고 지시하라. 이것은 취향이 아니라 `CLAUDE.md` "핵심 규칙"
   7번(코드 자립성) 위반이므로 아래 "보지 않는 것"에 해당하지 않는다.
   (`git diff`에서 해당 패턴을 훑어 확인하면 된다. `docs/` 아래 spec·ADR 문서의 ID는 정상이니 건드리지 마라.)
...
- `retryable_error` — 코드 수정으로 **명백히** 고쳐질 버그·회귀·테스트 누락·컨벤션 위반, 또는 위의 AC 정합 불일치.
